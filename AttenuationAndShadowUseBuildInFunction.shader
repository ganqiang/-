...
Shader "myshader/AttenuationAndShadowUseBuildInFunction"
{
	Properties
	{
	...
	SubShader
	{
		pass
		{
			...
			fixed4 frag(v2f i):SV_TARGET
			{
				...
				UNITY_LIGHT_ATTENUATION(atten, i, i.worldPos);//这个内置宏不仅计算衰减而且还计算阴影信息
				return fixed4(ambient + (diffuse + hightLight) * atten, 1);
			}
